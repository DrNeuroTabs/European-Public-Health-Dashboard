...
NEIGHBORS = {
    "AT":["DE","CZ","SK","HU","SI","IT"], "BE":["FR","DE","NL","LU"], "BG":["RO","EL"],
    "HR":["SI","HU"], "CY":[], "CZ":["DE","PL","SK","AT"], "DK":["DE"], "EE":["LV"],
    "FI":["SE"], "FR":["BE","LU","DE","IT","ES"], "DE":["DK","PL","CZ","AT","FR","LU","BE","NL"],
    "EL":["BG"], "HU":["AT","SK","RO","HR"], "IE":[], "IT":["FR","AT","SI"],
    "LV":["EE","LT"], "LT":["LV","PL"], "LU":["BE","DE","FR"], "MT":[],
    "NL":["BE","DE"], "PL":["DE","CZ","SK","LT"], "PT":["ES"], "RO":["BG","HU"],
    "SK":["CZ","PL","HU","AT"], "SI":["IT","AT","HU","HR"], "ES":["FR","PT"], "SE":["FI"]
}
SEX_NAME_MAP = {"T":"Total","M":"Male","F":"Female"}
REV_SEX_NAME = {v:k for k,v in SEX_NAME_MAP.items()}
CAUSE_NAME_MAP = {
    "TOTAL":"Total", "A_B":"Certain infectious and parasitic diseases (A00-B99)",
    # ... (all other codes as in prior script) ...
...
FACTOR_IDS = {
    "BMI by citizenship":"hlth_ehis_bm1c", "Phys activity by citizenship":"hlth_ehis_pe9c",
...
def load_eurostat_series(dataset_id:str)->pd.DataFrame:
    ...
    first=raw.columns[0];dims=first.split("\\")[0].split(",")
...
def forecast_mortality(df_sub:pd.DataFrame,periods:int,method:str):
...
def draw_directed_network(nodes,edges,title):
    ...
    if edges:
        nx.draw_networkx_edges(G,pos,arrows=True,arrowsize=20,width=2,edge_color='gray',
    ...
    for i,node in enumerate(nodes):
        ...
        deg=np.degrees(angle);ha="left" if np.cos(angle)>0 else "right"
        va="bottom" if np.sin(angle)>0 else "top"
        ax.text(x,y,node,ha=ha,va=va,rotation=deg,rotation_mode='anchor',
                fontsize=10,bbox=dict(facecolor='white',edgecolor='none',pad=0.3),zorder=3)
    top_label_y=max(np.sin(angles))*radius;title_y=top_label_y+0.15
    ax.set_title(title,y=title_y,pad=10);ax.set_axis_off();st.pyplot(fig)
...
def main():
    ...
    df=load_data();df["CountryFull"]=df["Country"].map(COUNTRY_NAME_MAP)
    df["CauseFull"]=df["Cause"].map(CAUSE_NAME_MAP);df["SexFull"]=df["Sex"].map(SEX_NAME_MAP)
    ...
    df_f=df[(df["Country"]==country_code)&(df["Cause"]==cause_code)&
            (df["Sex"].isin(sex_codes))&(df["Year"].between(*year_range))]
    if df_f.empty:
        st.warning("No data for selected filters.");joinpoint_df=pd.DataFrame()
        forecasts={}
    else:
        joinpoint_df=compute_joinpoints_and_apc(df_f)
        st.markdown("### Joinpoint Trend");plot_joinpoints_comparative(df_f,f"{cause_full} Trend by Sex")
        ...
        st.markdown("### Joinpoint & APC");st.dataframe(joinpoint_df,use_container_width=True)
    ...
    if factors:
        reg_min,reg_max=st.slider("Regression Years",min_value=year_range[0],
                                  max_value=year_range[1],value=(year_range[0],year_range[1]),
        ...
        if (reg_max-reg_min)<3:
            ...
            pf=all_f[(all_f["FactorName"].isin(factors))&
                     (all_f["Year"].between(reg_min,reg_max))&
                     (all_f["Sex"].isin(sex_codes))]
            pm=df[(df["Cause"]==cause_code)&(df["Year"].between(reg_min,reg_max))&
                  (df["Sex"].isin(sex_codes))][["Country","Year","Rate"]].rename(columns={"Rate":"Mortality"})
            panel=pf.pivot_table(index=["Country","Year"],columns="FactorName",values="Rate").reset_index().merge(pm,on=["Country","Year"],how="inner")
            present=[f for f in factors if f in panel.columns]
            missing=set(factors)-set(present)
            if missing: st.warning(f"Skipped unavailable factors: {', '.join(missing)}")
            if present:
                before=panel.shape[0]
                panel_clean=panel.dropna(subset=present+["Mortality"])
                dropped=before-panel_clean.shape[0]
                if dropped>0: st.warning(f"Dropped {dropped} observations due to missing data.")
                if panel_clean.shape[0]<len(present)*2:
                    st.warning("Not enough observations for reliable regression.")
                else:
                    X=sm.add_constant(panel_clean[present]);y=panel_clean["Mortality"]
                    mdl=sm.OLS(y,X).fit()
                    st.subheader("Regression summary");st.text(mdl.summary())
                    reg_coefs=mdl.params.drop("const")
                    st.plotly_chart(px.bar(x=reg_coefs.index,y=reg_coefs.values,
    ...
    pivot=df_cl.pivot(index="Country",columns="Year",values="Rate").interpolate(axis=1,limit_direction="both").ffill(axis=1).bfill(axis=1).dropna(axis=0,how="all")
    clust_df=pd.DataFrame()
    if pivot.shape[0]<3:
        st.warning("Not enough total-rate countries to cluster.")
    else:
        X=pivot.values;max_k=min(10,X.shape[0]-1)
        sil_scores={k:silhouette_score(X,KMeans(n_clusters=k,random_state=0).fit_predict(X)) for k in range(2,max_k+1)}
        sil_df=pd.Series(sil_scores,name="silhouette_score").to_frame()
        st.write("Silhouette by # clusters:",sil_df)
        st.plotly_chart(px.line(sil_df,x=sil_df.index,y="silhouette_score",labels={"index":"# clusters"},title="Silhouette vs # Clusters"))
        best_k=max(sil_scores,key=sil_scores.get);st.write(f"Optimal k: **{best_k}**")
        ...
        st.plotly_chart(px.choropleth(clust_df,locations="iso_alpha",color="Cluster",hover_name="CountryFull",
                                      locationmode="ISO-3",scope="europe",title=f"{cause_full} Clusters (k={best_k})"))
    ...
    if len(sel_countries)>=2:
        ...
        st.subheader("Bayes-factor matrix (both directions)");st.dataframe(bf_mat.style.format("{:.2f}"))
        for src in common:
            for dst in common:
                if src==dst: continue
                ...
                if pd.notna(bf) and bf>=bf_thresh:
                    ...
                    if dc in NEIGHBORS.get(sc,[]): edges_global.append((src,dst))
    ...
    st.plotly_chart(px.choropleth(map_df,locations="iso_alpha",color="Role",hover_name="CountryFull",
                                  locationmode="ISO-3",scope="europe",title="Focal & Neighbors"))
    ...
    if nbrs:
        ...
        st.subheader("Neighbor Bayes-factor matrix (both directions)");st.dataframe(bf_n.style.format("{:.2f}"))
        for src in common_n:
            for dst in common_n:
                if src==dst: continue
                bf=bf_n.loc[src,dst]
                if pd.notna(bf) and bf>=nbr_bf: edges_n.append((COUNTRY_NAME_MAP[src],COUNTRY_NAME_MAP[dst]))
    ...
    zip_buf.seek(0)
    st.download_button(label="Download current results report",
                       data=zip_buf,
